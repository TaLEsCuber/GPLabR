...
for file_name in file_names:
    if file_name.endswith('.xlsx'):
        ...
        df_range = df[(df['Wavelength [nm]'] >= 500) & (df['Wavelength [nm]'] <= 600)]
        # 找到吸收率最高的位置对应的波长和吸收率
        peaks, _ = find_peaks(df_range[absorbance_column], height=0, distance=20, prominence=0.1)  # 找到峰值
        peak_indices = peaks[df_range.iloc[peaks][absorbance_column].argsort()[-3:][::-1]]  # 找到最高的3个峰
        for index in peak_indices:
            peak_wavelength = df_range.iloc[index]['Wavelength [nm]']
            peak_absorbance = df_range.iloc[index][absorbance_column]
            # 绘制峰值
            plt.plot(peak_wavelength, peak_absorbance, 'ro')  # 红色圆点表示峰值
            # 在图上标出峰值对应的波长和吸收率
            plt.annotate(f"{peak_wavelength:.2f} nm, {peak_absorbance:.2f}",
                         xy=(peak_wavelength, peak_absorbance),
                         xytext=(peak_wavelength - 5, peak_absorbance - 0.2),
                         arrowprops=dict(arrowstyle='->', mutation_scale=10))
